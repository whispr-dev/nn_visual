...
class NetworkVisualizer:
    def __init__(self):
        ...
        # Adjust figure size and create more vertical space
        self.fig, self.ax = plt.subplots(figsize=(15, 15))
        self.ax.set_xlim(-0.1, 1.1)
        # Expand vertical limits to show more of the top layers
        ...
        self.input_pos = {f'i{i}': (0.1, 0.2 + i*0.06) for i in range(10)}
        
        # Three parallel pathways (adjusted vertical spread)
        # RNN moved down from 0.8
        self.rnn_pos = {f'rnn{i}': (0.3, 0.6 + i*0.02) for i in range(20)}
        # SOM centered
        self.som_pos = {f'som{i}': (0.3, 0.4 + i*0.02) for i in range(10)}
        # LSTM moved up from 0.2
        self.lstm_pos = {f'lstm{i}': (0.3, 0.25 + i*0.02) for i in range(20)}
        
        # Ganglion (adjusted spread)
        self.ganglion_pos = {
            f'g{i}': (0.6, 0.2 + i*0.02) for i in range(30)
        }
        
        # Output layer (adjusted spacing)
        ...
        self.ax.text(0.1, 0.15, 'Input Layer', ha='center')
        self.ax.text(0.3, 0.95, 'RNN', ha='center')
        self.ax.text(0.3, 0.45, 'SOM', ha='center')
        self.ax.text(0.3, 0.15, 'LSTM', ha='center')
